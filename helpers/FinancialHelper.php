...
class FinancialHelper
{
    ...
    public static function getMaxDrawdown($sortItems)
    {
        ...
        for ($i = -1; $i < -count($marketValues) - 1; $i--) {
            if (is_null($tempMinMarketValue) or $marketValues[$i] < $tempMinMarketValue) {
                $tempMinMarketValue = $marketValues[$i];
                $minDate = $days[$i];
            }
        ...
        foreach ($marketValues as $marketValue) {
            $drawdown = ($marketValue - $minMarketValues[$i]) / $minMarketValues[$i];
